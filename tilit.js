...
(function(){
  ...
  var A=window.HKAcct={
    ...
    saveSlots:function(arr){try{_lsSet(this.slotsKey(),JSON.stringify(arr));}catch(e){}
      if(this.db&&this.fbuid&&this.uid==='u_'+this.fbuid)this.db.collection('users').doc(this.fbuid).collection('meta').doc('slots').set({list:arr}).catch(function(){});},
    ensureSlot:function(){var s=this.getSlots();if(!s.length){s=[{id:this.slot||'1',name:'Peli 1',created:Date.now(),lastPlayed:Date.now()}];this.saveSlots(s);}
      if(!s.some(function(x){return x.id===A.slot;})){A.slot=s[0].id;_lsSet('hk_activeSlot',A.slot);}},
    ...
    deleteSlot:function(id){var s=this.getSlots().filter(function(x){return x.id!==id;});this.saveSlots(s);
      try{_lsDel('hatakeskus_code::'+this.uid+'::'+id);}catch(e){}
      try{if(window.storage&&window.storage.delete)window.storage.delete('hatakeskus_v2::'+this.uid+'::'+id);}catch(e){}
      if(this.db&&this.fbuid&&this.uid==='u_'+this.fbuid)this.db.collection('users').doc(this.fbuid).collection('saves').doc(id).delete().catch(function(){});},
    ...
    cloudPush:function(b64){if(!(this.db&&this.fbuid&&this.uid==='u_'+this.fbuid&&this.slot))return;var self=this;
      clearTimeout(this._pt);this._pt=setTimeout(function(){self.cloudDocSlot(self.slot).set({data:b64,ts:Date.now()}).catch(function(){});},10000);},
    cloudDeleteCur:function(){if(this.db&&this.fbuid&&this.uid==='u_'+this.fbuid&&this.slot)this.cloudDocSlot(this.slot).delete().catch(function(){});},
    ...
    signOut:function(){if(this.auth)this.auth.signOut();this.setActive('local',_lsGet('hk_lastLocalSlot')||'1');},
    ...
    render:function(){
      ...
      who.innerHTML=this.user
        ?'<span class="hk-dot ok"></span> Kirjautunut: <b>'+esc(this.user.displayName||this.user.email||'Google-tili')+'</b>'+(this.uid==='u_'+this.fbuid?' · pelaat Google-pelejä':' · pelaat paikallisia pelejä')
      ...
      if(!CONFIGURED){auth.innerHTML='<div class="hk-fb-off">&#9729; Google-synkronointi pois käytöstä. Pelit tallentuvat vain tähän laitteeseen. Ota Google käyttöön README-ohjeella.</div>';}
        ...
        var btn='';
        if(this.uid!=='u_'+this.fbuid)btn='<button class="hk-google" onclick="HKAcct.setActive(\'u_'+this.fbuid+'\',(HKAcct._firstCloudSlot||\'1\'))"><b>G</b> Siirry Google-peleihisi</button>';
        ...
        A.auth.onAuthStateChanged(function(u){
          if(u){
            ...
            A.db.collection('users').doc(u.uid).collection('meta').doc('slots').get().then(function(d){
              if(d.exists&&d.data().list){try{_lsSet('hk_slots::u_'+u.uid,JSON.stringify(d.data().list));}catch(e){}var l=d.data().list;A._firstCloudSlot=(l[0]&&l[0].id)||'1';}
  ...
  function injectUI(){
    ...
    css.textContent=`
#hkAccount{position:fixed;inset:0;z-index:5000;background:radial-gradient(circle at 30% 18%,#12161c,#0a0c0f 72%);display:none;align-items:center;justify-content:center;padding:18px;font-family:var(--fui,system-ui)}
#hkAccount .hk-card{width:100%;max-width:440px;background:var(--bg2,#12161c);border:1px solid var(--line2,#3a4b5c);border-radius:14px;box-shadow:0 24px 70px #000a;overflow:hidden;max-height:92vh;display:flex;flex-direction:column}
#hkAccount .hk-top{padding:18px 20px 14px;border-bottom:1px solid var(--line2,#3a4b5c);background:linear-gradient(180deg,#141a21,#0e1217);position:relative;display:flex;align-items:center}
#hkAccount .hk-top:before{content:"";position:absolute;left:0;top:0;bottom:0;width:4px;background:var(--acc,#ffb01f);box-shadow:0 0 12px var(--acc,#ffb01f)}
#hkAccount .hk-logo{font-family:var(--fdisp,sans-serif);font-weight:700;letter-spacing:3px;font-size:22px;text-transform:uppercase;color:var(--txt,#eef3f7);flex:1}
#hkAccount .hk-x{width:34px;height:34px;border:1px solid var(--line2,#3a4b5c);background:var(--bg3,#1a2129);color:var(--dim,#8696a6);border-radius:7px;cursor:pointer;font-size:16px}
#hkAccount .hk-who{padding:11px 20px;font-size:13px;color:var(--dim,#8696a6);border-bottom:1px solid var(--line,#2b3947);display:flex;align-items:center;gap:8px}
#hkAccount .hk-who b{color:var(--txt,#eef3f7)}
#hkAccount .hk-dot{width:9px;height:9px;border-radius:50%;background:#5d6b79;flex:none}
#hkAccount .hk-dot.ok{background:var(--acc2,#26d07c);box-shadow:0 0 8px var(--acc2,#26d07c)}
#hkAccount .hk-body{padding:14px 16px;overflow-y:auto;flex:1}
#hkAccount .hk-slot{display:flex;align-items:center;gap:6px;background:var(--bg3,#1a2129);border:1px solid var(--line,#2b3947);border-left:4px solid var(--acc,#ffb01f);border-radius:8px;padding:10px 11px;margin-bottom:8px}
#hkAccount .hk-slot.hk-active{border-left-color:var(--acc2,#26d07c);background:#16241c}
#hkAccount .hk-slot-main{flex:1;min-width:0;cursor:pointer}
#hkAccount .hk-slot-name{font-weight:700;font-size:15px;color:var(--txt,#eef3f7)}
#hkAccount .hk-now{color:var(--acc2,#26d07c);font-weight:600;font-size:12px}
#hkAccount .hk-slot-meta{font-size:11.5px;color:var(--dim,#8696a6);font-family:var(--fmono,monospace);margin-top:2px}
#hkAccount .hk-mini{width:38px;height:38px;flex:none;border:1px solid var(--line2,#3a4b5c);background:var(--bg2,#12161c);color:var(--dim,#8696a6);border-radius:7px;cursor:pointer;font-size:15px}
#hkAccount .hk-mini:active{background:var(--acc,#ffb01f);color:#1a1200}
#hkAccount .hk-mini.hk-danger:active{background:#ff3b3b;color:#fff}
#hkAccount .hk-empty{color:var(--dim,#8696a6);text-align:center;padding:26px 8px;font-size:13.5px}
#hkAccount .hk-foot{padding:12px 16px;border-top:1px solid var(--line2,#3a4b5c);background:#0e1217}
#hkAccount .hk-new{width:100%;background:var(--acc2,#26d07c);color:#06210f;border:none;border-radius:9px;padding:14px 0;font-weight:800;font-size:16px;letter-spacing:1px;text-transform:uppercase;cursor:pointer;box-shadow:0 0 18px #26d07c44}
#hkAccount .hk-google{width:100%;display:flex;align-items:center;justify-content:center;gap:9px;background:#fff;color:#222;border:none;border-radius:9px;padding:12px 0;font-weight:700;font-size:14px;cursor:pointer;margin-top:10px}
#hkAccount .hk-google b{display:inline-flex;width:22px;height:22px;align-items:center;justify-content:center;background:#4285F4;color:#fff;border-radius:50%;font-family:serif;font-weight:700}
#hkAccount .hk-line{width:100%;background:var(--bg3,#1a2129);color:var(--dim,#8696a6);border:1px solid var(--line2,#3a4b5c);border-radius:8px;padding:10px 0;cursor:pointer;font-size:13px;margin-top:10px}
#hkAccount .hk-fb-off{background:#1a1407;border:1px solid var(--acc,#ffb01f);border-radius:8px;padding:10px;color:#ffe9c2;font-size:12px;line-height:1.5;margin-top:10px}
#hkAccount .hk-note{color:var(--dim,#8696a6);font-size:11.5px;text-align:center;margin-top:7px;line-height:1.5}
#hkAccNote{color:#ff9a6a;font-size:12px;text-align:center;margin-top:8px;min-height:14px}
#hkPelitCard{border:1px solid var(--acc2,#26d07c);border-radius:12px;padding:16px;margin-bottom:14px;background:linear-gradient(180deg,#11211a,#0e1614);font-family:var(--fui,system-ui)}
#hkPelitCard .hk-pel-h{font-family:var(--fdisp,sans-serif);font-weight:800;letter-spacing:2px;font-size:17px;color:var(--txt,#eef3f7);margin-bottom:8px}
#hkPelitCard .hk-pel-sub{color:var(--dim,#8696a6);font-size:13px;line-height:1.55;margin-bottom:14px}
#hkPelitCard .hk-pel-btn{width:100%;background:var(--acc2,#26d07c);color:#06210f;border:none;border-radius:9px;padding:14px 0;font-weight:800;font-size:15px;letter-spacing:.5px;cursor:pointer;box-shadow:0 0 18px #26d07c44;font-family:var(--fui,sans-serif)}
#hkPelitCard .hk-pel-btn:active{filter:brightness(.92)}`;
    document.head.appendChild(css);
    var ov=document.createElement('div');ov.id='hkAccount';
    ov.innerHTML=`<div class="hk-card">
      <div class="hk-top"><span class="hk-logo">HÄTÄKESKUS</span><button class="hk-x" onclick="HKAcct.close()">&times;</button></div>
      <div class="hk-who" id="hkAccWho"></div>
      <div class="hk-body" id="hkAccSlots"></div>
      <div class="hk-foot">
        <button class="hk-new" onclick="HKAcct.uiNew()">+ Uusi peli</button>
        <div id="hkAccAuth"></div>
        <div id="hkAccNote"></div>
      </div>
    </div>`;
    document.body.appendChild(ov);
  }

  /* ---- ASETUKSET: korvaa pelin oma pilvisynkronointi-osio "Pelit"-napilla ---- */
  function findCloudCard(){
    var els=document.querySelectorAll('div'), best=null;
    for(var i=0;i<els.length;i++){
      var t=els[i].textContent||'';
      if(t.indexOf('PILVISYNKRONOINTI')>=0 && t.indexOf('Liittyminen korvaa')>=0){
        if(!best || t.length<(best.textContent||'').length) best=els[i];
      }
    }
    return best;
  }
  function makePelitCard(){
    var card=document.createElement('div');
    card.id='hkPelitCard';
    card.innerHTML='<div class="hk-pel-h">👤 PELIT</div>'
      +'<div class="hk-pel-sub">Useita erillisiä pelejä samalla laitteella. Voit luoda, vaihtaa, nimetä ja poistaa pelejä. Kirjautumalla Google-tilillä pelit seuraavat sinua laitteelta toiselle.</div>'
      +'<button class="hk-pel-btn" onclick="HKAcct.open()">👤 Avaa pelit / vaihda peliä</button>';
    return card;
  }
  function injectSettings(){
    if(document.getElementById('hkPelitCard'))return; /* jo tehty */
    var card=findCloudCard();
    if(!card)return;
    card.style.display='none';
    var pel=makePelitCard();
    card.parentNode.insertBefore(pel,card);
  }

  function boot(){
    injectUI();
    initFirebase();
    /* Yritetään asettaa Pelit-nappi asetuksiin, ja korjataan itsestään jos asetukset piirretään uudelleen */
    injectSettings();
